...
def enhanced_analyze_job_ad(job_ad, file, ask_expert=False, expert_question=""):
    """Rozszerzona analiza z opcjonalną konsultacją eksperta"""
    
    # Podstawowa analiza
    json_output, word_file, short_word_file = analyze_job_ad(job_ad, file)
    
    expert_response = ""
    
    if ask_expert and expert_question.strip():
        if not HR_ASSISTANT_AVAILABLE or hr_assistant is None:
            expert_response = "⚠️ Ekspert HR nie jest dostępny. Sprawdź konfigurację modułu hr_assistant i klucz OPENAI_API_KEY."
        else:
            try:
                # Pobierz kontekst z podstawowej analizy dla lepszej odpowiedzi eksperta
                context_from_analysis = ""
                if isinstance(json_output, dict):
                    if "stanowisko" in json_output:
                        context_from_analysis += f"Stanowisko: {json_output['stanowisko']}\n"
                    if "wymagania" in json_output:
                        context_from_analysis += f"Wymagania: {json_output['wymagania']}\n"
                
                # Przygotuj pytanie z kontekstem
                full_context = f"""
Analizuję ogłoszenie o pracę o następujących charakterystykach:
{context_from_analysis}

Pytanie: {expert_question}
"""
                
                response = hr_assistant.ask(full_context)
                expert_response = f"🤖 **Ekspert HR:**\n\n{response['answer']}"
                
                # Dodaj informacje o źródłach jeśli są dostępne
                if response.get('sources'):
                    expert_response += f"\n\n📚 **Źródła:**\n"
                    for i, source in enumerate(response['sources'][:3], 1):  # Max 3 źródła
                        expert_response += f"{i}. {source.get('filename', 'Nieznany')} (str. {source.get('page', '?')})\n"
                
            except Exception as e:
                expert_response = f"❌ Wystąpił błąd podczas konsultacji z ekspertem: {str(e)}"
    elif ask_expert and not expert_question.strip():
        expert_response = "❓ Proszę wprowadzić pytanie do eksperta HR."
    
    return json_output, word_file, short_word_file, expert_response